...
제21조(연결“몰”과 피연결“몰” 간의 관계)
  ...
  ① 상위 “몰”과 하위 “몰”이 하이퍼링크(예: 하이퍼링크의 대상에는 문자, 그림 및 동화상 등이 포함됨)방식 등으로 연결된 경우, 전자를 연결 “몰”(웹 사이트)이라고 하고 후자를 피…
`;
const rules = `개인정보처리방침
...
export default function SellerForm({ toggleModal }: props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    await updateDb({ Route: `user/${uid}`, data: { seller: true } });
    toggleModal(null);
  };

  return (
    <div className="py-10 z-20 fixed w-[50%] min-w-[250px] max-w-[800px] max-h-[80%] h-full bg-[#f5f5f5] top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%]">
      <MdCancel
      ...
      <form
        onSubmit={onSubmit}
        className="flex flex-col items-center w-full h-full px-4 pb-4 mx-auto text-xs sm:text-sm "
      >
        <h1 className="w-full mb-3 text-2xl font-extrabold text-left uppercase select-none sm:text-4xl ">
          판매회원 약관
        </h1>
        <div className="flex w-full py-4 ">
          <label htmlFor="sellerRules" className="w-full ">
            이용약관 <strong className="text-red-600">(필수)</strong>
          </label>
          <input
        ...
        <textarea
          value={sellerRules}
          readOnly
          className="outline-blue-400 w-full h-full max-h-[200px] break-words resize-none overflow-auto "
        ></textarea>
        <div className="flex w-full py-4 ">
          <label htmlFor="rules" className="w-full ">
            개인정보 수집 및 이용 동의{" "}
            <strong className="text-red-600">(필수)</strong>
          </label>
          <input className="ml-auto mr-4" required id="rules" type="checkbox" />
        </div>
        <textarea
          value={rules}
          readOnly
        ...
        <button
          type="submit"
          className="mb-1 mt-10 duration-300 relative w-full max-w-[40ch] border bg-white border-white border-solid uppercase py-2 overflow-hidden
    ...
    hover:after:translate-x-full hover:text-slate-900 "
        >
          <h2 className="relative z-20">확인</h2>
        </button>
      </form>
    </div>
  );
